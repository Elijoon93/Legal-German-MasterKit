...
(function(){
  ...
  function finalCheck(){
    ...
    showBootError(last?.text||"فایل‌های برنامه کامل اجرا نشدند. صفحه را یک‌بار بازآوری کنید.");
    const retry=document.querySelector("#retryBtn");
    if(retry)retry.onclick=()=>location.replace(`${location.pathname}?v=940&t=${Date.now()}`);
  }
