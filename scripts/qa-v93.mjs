...
const required=[
  'assets/js/v93-learning-os.js','assets/css/v93-learning-os.css','release-v9.3.json',
...
for(const token of ['Legal German MasterKit v9.3.0','manifest.webmanifest?v=930','LGMK_RELEASE_VERSION="9.3.0"','v93-learning-os.css?v=930','v93-learning-os.js?v=930','id="deviceAcceptance"'])index.includes(token)?pass('index token',token):fail('index token',token);
...
index.indexOf('v93-learning-os.js?v=930')>index.indexOf('v92-device.js?v=920')?pass('v9.3 runtime precedence'):fail('v9.3 runtime precedence');
...
for(const token of ['const VERSION="9.3.0"','const COMPETENCIES=[','const STAGES=[','evidence[item.id]*.85','(self*20)*.15','v93StartFocus','v93Diagnostic','v93ToolsOverlay','completionCreatesEvidence'.replace('completionCreatesEvidence','state.v93.evidence.unshift')])runtime.includes(token)?pass('learning OS token',token):fail('learning OS token',token);
...
for(const token of ['lgmk-v9-3-dual-reference-learning-os','release-v9.3.json','v93-learning-os.css?v=930','v93-learning-os.js?v=930'])sw.includes(token)?pass('cache token',token):fail('cache token',token);

if(failed){console.error(`\n${failed} cumulative v9.3 gate(s) failed.`);process.exit(1)}
console.log('\nAll cumulative v9.3 dual-reference learning OS, content and adaptive device gates passed.');
